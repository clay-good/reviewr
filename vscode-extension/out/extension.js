...
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('reviewr.reviewCurrentFile', () => {
        reviewCurrentFile();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('reviewr.reviewWorkspace', () => {
        reviewWorkspace();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('reviewr.clearDiagnostics', () => {
    ...
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider({ scheme: 'file' }, new ReviewrCodeActionProvider(), {
    ...
    context.subscriptions.push(vscode.languages.registerHoverProvider({ scheme: 'file' }, new ReviewrHoverProvider()));
...
async function reviewPath(targetPath) {
    const config = vscode.workspace.getConfiguration('reviewr');
    const cliPath = config.get('cliPath', 'reviewr');
    const useAllTypes = config.get('useAllReviewTypes', true);
    const reviewTypes = config.get('reviewTypes', ['security', 'performance', 'correctness']);
    const clearProblems = config.get('clearProblemsOnReview', true);
...
function processSarifResults(sarif) {
    ...
    for (const result of results) {
        ...
        const diagnostic = new vscode.Diagnostic(new vscode.Range(region.startLine - 1, region.startColumn || 0, region.endLine - 1, region.endColumn || Number.MAX_SAFE_INTEGER), result.message.text, mapSeverity(result.level));
        ...
        if (result.fixes && result.fixes.length > 0) {
            ...
            diagnostic.relatedInformation = [
                new vscode.DiagnosticRelatedInformation(new vscode.Location(vscode.Uri.file(filePath), diagnostic.range), `💡 Suggestion: ${fix.description.text}`)
            ];
        }
...
function mapSeverity(sarifLevel) {
    switch (sarifLevel) {
        case 'error':
...
class ReviewrCodeActionProvider {
    provideCodeActions(document, range, context, token) {
        ...
        for (const finding of findings) {
            ...
            const showDetailsAction = new vscode.CodeAction('📖 Show Details', vscode.CodeActionKind.QuickFix);
            ...
            if (finding.result.fixes && finding.result.fixes.length > 0) {
                ...
                const fixAction = new vscode.CodeAction(`💡 ${fix.description.text}`, vscode.CodeActionKind.QuickFix);
...
ReviewrCodeActionProvider.providedCodeActionKinds = [
    vscode.CodeActionKind.QuickFix
];
// Hover Provider for detailed information
class ReviewrHoverProvider {
